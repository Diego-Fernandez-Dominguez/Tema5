...
public class MandoMain {
	public static void main(String[] args) {
		...
		Mando mand1;
		try {
			mand1 = new Mando("Mando1", 34, 65, 100);
			mandos.add(mand1);
			...
			mandos.add(ac2);
		} catch (ModeloException | PrecioException e) {
		...
		Comparator<Mando> precioMayor = (a, b) -> {
			return (int) (a.getPrecio() - b.getPrecio());
		};

		Collections.sort(mandos, precioMayor);
		System.out.println(mandos);

		// Altura Descendidente
		Collections.sort(mandos, (a, b) -> (int) (b.getAltura() - a.getAltura()));
		System.out.println(mandos);

		/*
		 * for (Mando mand : mandos) {
		 * 
		 * if (mand instanceof Television) { mand.encenderMando(); ((Television)
		 * mand).setCanal(7); ((Television) mand).subirVolumen(); ((Television)
		 * mand).bajarVolumen(); System.out.println(mand);
		 * 
		 * } else if (mand instanceof MiniCadena) { mand.encenderMando(); ((MiniCadena)
		 * mand).subirVolumen(); ((MiniCadena) mand).bajarVolumen();
		 * System.out.println(mand);
		 * 
		 * } else if (mand instanceof Aspiradora) { ((AireAcondicionado)
		 * mand).subirVelocidad(); ((AireAcondicionado) mand).bajarVelocidad();
		 * System.out.println(mand);
		 * 
		 * } else if (mand instanceof AireAcondicionado) { mand.encenderMando();
		 * ((AireAcondicionado) mand).setTipoModo("CALOR"); ((AireAcondicionado)
		 * mand).setTemperatura(5); ((AireAcondicionado) mand).subirVelocidad();
		 * ((AireAcondicionado) mand).bajarVelocidad(); System.out.println(mand);
		 * 
		 * }
		 * 
		 * }
